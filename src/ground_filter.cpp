...
: Node(node_name, ns, options)
{
  ...
  cloth_points_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>("~/cloth_points", 1);
...
void GroundFilter::pointsCallback(sensor_msgs::msg::PointCloud2::UniquePtr msg)
{
  ...
  if (publish_cloth_points_) {
    auto cloth_points_msg = std::make_unique<sensor_msgs::msg::PointCloud2>();
    pcl::toROSMsg(*csf_->getClothPoints(), *cloth_points_msg);
    cloth_points_msg->header = msg->header;
    if (!gravity_aligned_frame_.empty()) {
      cloth_points_msg->header.frame_id = gravity_aligned_frame_;
    }
    cloth_points_pub_->publish(std::move(cloth_points_msg));
  }
}
...
void GroundFilter::initializeParameters()
{
  ...
  publish_off_ground_points_ = declare_parameter<bool>("publish_off_ground_points", false);
  publish_cloth_points_ = declare_parameter<bool>("publish_cloth_points", false);

  csf_ = std::make_unique<csf_pcl::ClothSimulationFilter<PCLPoint>>(
    publish_off_ground_points_, publish_cloth_points_);
  csf_->setClothResolution(
    declare_parameter<double>("cloth_resolution", csf_->getClothResolution()));
  ...
  parameter_callback_handles_.push_back(parameter_event_handler_->add_parameter_callback(
    "time_step",
    [this](const rclcpp::Parameter & param) { csf_->setTimeStep(param.as_double()); }));
  parameter_callback_handles_.push_back(parameter_event_handler_->add_parameter_callback(
    "max_iterations",
  ...
  parameter_callback_handles_.push_back(parameter_event_handler_->add_parameter_callback(
    "slope_fitting_threshold", [this](const rclcpp::Parameter & param) {
      csf_->setSlopeFittingThreshold(param.as_double());
    }));
  parameter_callback_handles_.push_back(parameter_event_handler_->add_parameter_callback(
